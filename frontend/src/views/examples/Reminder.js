...
const Profile = () => {
  ...
  //Route  for filter and seaRCH date and  option employee field
  const handleEmployeeChange = (event) => {
    setSelectedEmployeeOption(event.target.value);
  };

  const handleDateEmployeSearch = async (event) => {
    event.preventDefault();
    try {
      const employeeSearchResult = await axios.get(`http://localhost:5000/search-reminder-employee?reminderDate=${selectedDate}&employee=${selectedEmployeeOption}&tablename=tbl_history`)
      console.log("Searching results for employeeSearchResult is : ", employeeSearchResult);

      if (employeeSearchResult.data && employeeSearchResult.data.length > 0) {
        setUserdata(employeeSearchResult.data);
        setTotalRecords(employeeSearchResult.data.length);
      }
      else {
      ...
      console.log("Error occured in fetching reminder-epmployee with date results:", error);
    }
  }
  ...
  // route for showing data of Reminder-manage
              ...
              <CardBody>
                <div className="pl-lg-4">
                  <Row className="form-control-main-section">
                    <Col lg="6">
                      < Form onSubmit={handleDateEmployeSearch} style={{ display: "flex" }}>
                        <Col lg="4">
                        ...
                        <Col lg="6">
                          <FormGroup>
                            <label className="form-control-label" htmlFor="input-email" > Care Team </label>
                              ...
                              <select id="option" value={selectedEmployeeOption} onChange={handleEmployeeChange} className="form-control-alternative">
                        ...
                        <div> <Button className="form-control-table-inner-button" type="submit" style={{marginTop:"30px"}}>Submit </Button></div>
                      </Form>
                    </Col>
